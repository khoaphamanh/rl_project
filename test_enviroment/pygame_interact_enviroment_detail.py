...
# ---- third column: raw + decoded value of every observation channel ----
CHANNEL_PANEL_WIDTH = 380
CH_CELL_W = 44  # px per cell in the channel tables
CH_CELL_H = 26
CH_ROW_LABEL_W = 28

# axis 2 of obs['image']: (channel index, name, table used to decode it)
CHANNELS = [
    (0, "object id", IDX_TO_OBJ),
    (1, "color id", IDX_TO_COLOR_NAME),
    (2, "state id", IDX_TO_STATE),
]
...
def draw_obs_grid(screen, image, ox, oy, font_tiny):
    ...
    for row in range(n):
        for col in range(n):
            ...
            rect = pygame.Rect(
                ox + col * CELL_SIZE, oy + row * CELL_SIZE, CELL_SIZE - 1, CELL_SIZE - 1
            )
            ...
            if label:
                ...
                screen.blit(
                    surf,
                    (
                        rect.x + (CELL_SIZE - 1 - surf.get_width()) // 2,
    ...
    pygame.draw.polygon(
        screen, (255, 255, 80), [(ax, ay - 7), (ax - 5, ay), (ax + 5, ay)]
    )
...
def interpret_obs(image):
    ...
    for row in range(n):
        for col in range(n):
            ...
            side = (
                "center"
                if lateral == 0
                else f"{abs(lateral)} {'left' if lateral < 0 else 'right'}"
            )
            if steps_fwd > 0:
    ...
    if agent_row > 0:
        ...
        if fi == 2:
            ...
            lines.insert(
                0,
                f">> {fcol.capitalize()} door ahead ({door_s})"
                f" — press Space to {'open' if door_s != 'open' else 'close'}",
            )
        elif fi in (5, 6, 7):
            lines.insert(
                0, f">> {fcol.capitalize()} {fname} directly ahead — press P to pick up"
            )
        elif fi == 8:
            lines.insert(0, ">> GOAL tile directly ahead — move forward to reach it!")
        elif fi == 9:
            lines.insert(
                0, ">> LAVA directly ahead — stepping forward will end the episode!"
            )
...
def draw_legend(screen, ox, y, font_tiny):
    ...
    for label, color, name in LEGEND:
        ...
        if label:
            ...
            screen.blit(
                ls, (x + (13 - ls.get_width()) // 2, y + (13 - ls.get_height()) // 2)
            )
...
# ---------------------------------------------------------------------------
# Channel value panel (third column)
# ---------------------------------------------------------------------------
...
def draw_channel_block(screen, ox, y, image, channel, name, table, font_tiny, font_micro):
    ...
    rows = y (0 = farthest ahead) and cols = x (3 = straight ahead).
    """
    n = image.shape[0]
    ...
    gx = ox + CH_ROW_LABEL_W
    for col in range(n):
        t = font_micro.render(f"x={col}", True, (120, 120, 120))
        screen.blit(t, (gx + col * CH_CELL_W + (CH_CELL_W - t.get_width()) // 2, y))
    y += 12

    for row in range(n):
        rl = font_micro.render(f"y={row}", True, (120, 120, 120))
        screen.blit(rl, (ox, y + (CH_CELL_H - rl.get_height()) // 2))

        for col in range(n):
            value = int(image[col, row, channel])
            obj_idx = int(image[col, row, 0])
            # Dim the cells where the number is meaningless filler:
            #   colour: 0 on unseen/empty cells, decodes to "red" but means nothing
            #   state : only a door actually has a state; elsewhere it is always 0
            if channel == 1:
                padding = obj_idx in (0, 1)
            elif channel == 2:
                padding = obj_idx != 4
            else:
                padding = False

            rect = pygame.Rect(gx + col * CH_CELL_W, y, CH_CELL_W - 2, CH_CELL_H - 2)
            pygame.draw.rect(screen, (30, 30, 34) if padding else (46, 48, 56), rect)
            if row == agent_row and col == agent_col:
                pygame.draw.rect(screen, (255, 255, 80), rect, 1)

            num_col = (85, 85, 85) if padding else (240, 240, 240)
            name_col = (75, 75, 75) if padding else (150, 205, 150)
...
def draw_channel_panel(screen, ox, win_h, image, font, font_tiny, font_micro):
    """Third column: every value of obs['image'], raw and decoded."""
    pygame.draw.rect(screen, (20, 20, 20), (ox, 0, CHANNEL_PANEL_WIDTH, win_h))
    pad = ox + 10
    y = 10

    t = font.render("CHANNEL VALUES  obs['image'][x, y, c]", True, (255, 210, 80))
    screen.blit(t, (pad, y))
    y += t.get_height() + 4
    for line in (
        f"shape {image.shape} = {image.size} uint8 numbers, axis 2 is NOT RGB",
        "rows = y: 0 = farthest ahead, 6 = agent's own row",
        "cols = x: 0 = far left, 3 = straight ahead, 6 = far right",
    ):
        s = font_micro.render(line, True, (140, 140, 140))
        screen.blit(s, (pad, y))
        y += s.get_height() + 1
    y += 6

    for channel, name, table in CHANNELS:
        y = draw_channel_block(
            screen, pad, y, image, channel, name, table, font_tiny, font_micro
        )
        y += 10
    ...
    for line in (
        ...
        s = font_micro.render(line, True, (120, 120, 120))
        screen.blit(s, (pad, y))
        y += s.get_height() + 1
...
def draw_sidebar(
    ...
    def divider():
        nonlocal y
        y += 5
        pygame.draw.line(screen, (65, 65, 65), (sx + 5, y), (sx + SIDEBAR_WIDTH - 5, y))
        y += 7
    ...
    pygame.draw.line(
        screen, (65, 65, 65), (sx + 5, ctrl_y), (sx + SIDEBAR_WIDTH - 5, ctrl_y)
    )
    ctrl_y += 6
...
def main():
    ...
    win_w = disp_w + SIDEBAR_WIDTH + CHANNEL_PANEL_WIDTH
    ...
    font_micro = pygame.font.SysFont("monospace", 9)
    ...
    while running:
        ...
        if action is not None:
            ...
            status = (
                "TERMINATED" if terminated else "TRUNCATED" if truncated else "ongoing"
            )
            print(
                f"Step {step:03d} | {ACTION_NAMES[action]:<14} | "
            ...
            if terminated or truncated:
                ...
                message = (
                    f"EPISODE {result} — total reward={total_reward:.3f}  "
