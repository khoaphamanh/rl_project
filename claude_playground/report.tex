...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.68\linewidth]{task_memory_s17.png}\\[6pt]
    \includegraphics[width=0.96\linewidth]{model_architecture.png}
...
\paragraph{Why eight steps suffice for a seventeen-step solution.} A solving
episode lasts $16.8$ steps, so $L=8$ spans only about half of it and still loses
nothing to full BPTT. Three things reconcile this. First, BPTT is not the path
that carries reward credit: GAE bootstraps through the value function over the
whole episode irrespective of $L$ (Eq.~\ref{eq:gae}), so the backward window has
only to shape the encoder's representation, namely writing the cue into $h$,
holding it, and reading it at the fork. Second, only part of the trajectory has
to be held: the steps before the agent turns around and sights the cue need no
memory at all, so the interval to be spanned is the return leg rather than the
episode. Third, chunks tile the episode and each is seeded with the recorded $h$,
so every step still receives gradient in every epoch and neighbouring windows
hand credit across their shared boundary over successive updates; a single window
therefore has only to teach the local behaviour of leaving $h$ intact while
nothing relevant is in view, which the forward pass carries arbitrarily far. The
equality of $L=8$ and full BPTT is the evidence for this reading: the longer
reach is available to the full-BPTT arm and changes neither its score nor its
$16.8$ steps, so the bar to clear is the effective gradient horizon and not the
episode length.
...
follow directly: sweep $L \in \{5,6,7\}$, log the distance from the cue sighting
to the fork, and vary the corridor length, which together would turn the reading
of Section~4 into a measurement; add R2D2-style burn-in \citep{r2d2}, which
